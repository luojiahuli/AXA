# A Gradient Boosting Model with added features. 
...
for(driver in drivers)
{
        ...
        g = gbm(ntarget ~., data = train, n.trees = 1000, distribution = "bernoulli", shrinkage = 0.01, interaction.depth = 4)
        currentData = as.data.frame(currentData)
        names(currentData) = paste0("n",names(currentData))
        p =predict(g, currentData, n.trees = 1000, type = "response")
...
write.csv(submission, "kaggle/Axa/submissionGBM.csv", row.names=F, quote=F)
